...
test("학생 기록과 교사 활동지는 DOCX 저장과 Google 문서 가져오기를 함께 제공한다", () => {
  assert.match(source, /buildStudentNotebookDocx/u);
  assert.match(source, /buildTeacherActivityDocx/u);
  assert.match(source, /buildGoogleDocsImportHtml/u);
  assert.match(source, /filename: "climate-exploration-note\.docx"/u);
  assert.match(source, /filename: "climate-class-activity\.docx"/u);
  assert.match(source, /googleDocsFilename: "climate-exploration-note-google-docs\.html"/u);
  assert.match(source, /googleDocsFilename: "climate-class-activity-google-docs\.html"/u);
  assert.match(source, /label: "Google 문서로 가져오기"/u);
  assert.match(source, /내 드라이브에서 Google 문서로 열기/u);
...
test("CSV는 스프레드시트 공유를, HTML은 대화형 원본의 Drive 공유를 제공한다", () => {
  ...
  assert.match(source, /브라우저 보안 정책 때문에 이 파일을 바로 공유할 수 없습니다/u);
});
