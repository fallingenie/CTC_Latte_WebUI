...
test("자료 내보내기는 허용하지 않은 출처 문구가 있으면 파일 생성을 중단한다", () => {
  ...
  assert.throws(() => buildClimateCsv({
    ...validSeriesResponse,
    datasetUpdatedAt: "file:///private/dataset"
  }), /자료 갱신 시각/u);
});
...
test("연구용 CSV는 생성 시각과 자료 출처 및 월별 체감 기준을 보존한다", () => {
  ...
  assert.equal(summerCells[19], "2026-07-13T00:00:00Z");
...
test("CSV 자료 갱신 시각은 Backend 계약의 Z와 +00:00 UTC 표기를 모두 허용한다", () => {
  const updatedAtWithOffset = "2026-07-15T01:02:03.123456+00:00";
  const csv = buildClimateCsv({
    ...validSeriesResponse,
    datasetUpdatedAt: updatedAtWithOffset
  });
  assert.match(csv, new RegExp(updatedAtWithOffset.replace("+", "\\+"), "u"));
});
