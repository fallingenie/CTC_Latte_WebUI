...
import { validatePublicObservationAttribution } from "../source/runtime-policy.js";
import { ProductionDeploymentError } from "./start-production-gateway.mjs";

export const RELEASE_POINTER_SCHEMA_VERSION = 1;
export const GCS_CURRENT_DATASET_MODE = "gcs-current";
...
const ATTRIBUTION_READER_PATH = fileURLToPath(
  new URL("./read_ctwebui_attribution.py", import.meta.url)
);
...
export async function resolveCurrentGcsDataEnvironment(
  env = process.env,
  {
    fileSystem = fs,
    platform = process.platform,
    loadAttribution = loadMountedObservationAttribution
  } = {}
) {
  if (optionalText(env.CTC_RC_DATA_MODE) !== GCS_CURRENT_DATASET_MODE) {
    throw new ProductionDeploymentError("현재 GCS 자료 소비 모드가 명시되지 않았습니다.");
  }
  ...
  const expectedDatasetVersion = requiredSha256(
    env.CTC_RC_EXPECTED_DATASET_VERSION,
    "CTC_RC_EXPECTED_DATASET_VERSION"
  );
  if (!isAbsolutePathForPlatform(mountRoot, platform)
    || !isAbsolutePathForPlatform(webDataRoot, platform)) {
    throw new ProductionDeploymentError("GCS 마운트와 WebUI 자료 경로는 절대경로여야 합니다.");
  }
  if (!webDataRoot.toLowerCase().endsWith(".ctwebui")) {
    throw new ProductionDeploymentError("WebUI 자료 경로는 .ctwebui 디렉터리여야 합니다.");
  }
  if (!isPathWithin(mountRoot, webDataRoot, platform)) {
    throw new ProductionDeploymentError("WebUI 자료는 지정된 GCS 마운트 안에 있어야 합니다.");
  }
  ...
  if (!isPathWithin(mountRealPath, webDataRealPath, platform)) {
    throw new ProductionDeploymentError("WebUI 자료의 실제 경로가 GCS 마운트를 벗어났습니다.");
  }

  const [manifest, completion] = await Promise.all([
    readJsonObject(fileSystem, path.join(webDataRealPath, "manifest.json"), "WebUI 자료 manifest"),
    readJsonObject(
      fileSystem,
  ...
  validateCurrentGcsDataset({ manifest, completion });

  const actualDatasetVersion = await computeMountedDatasetVersion(webDataRealPath, { fileSystem });
  if (actualDatasetVersion !== expectedDatasetVersion) {
    throw new ProductionDeploymentError("현재 GCS 자료판 identity가 배포 시 확인한 값과 일치하지 않습니다.");
  }
  const attributionCatalog = await loadAttribution(webDataRealPath, { env, platform });
  ...
  return Object.freeze({
    ...
    pointer: Object.freeze({
      ...
      releaseId: `rc-${actualDatasetVersion.slice(0, 12)}`,
      relativePath,
...
export function validateCurrentGcsDataset({ manifest, completion }) {
  if (!isPlainRecord(manifest) || !isPlainRecord(completion)) {
    throw new ProductionDeploymentError("WebUI 자료 설명 형식이 올바르지 않습니다.");
  }
  if (manifest.format !== "Climate Time Capsule WebUI Hybrid Export"
    ...
    throw new ProductionDeploymentError("읽을 수 있는 WebUI 자료 형식이 아닙니다.");
  }
  return Object.freeze({
...
export async function loadMountedObservationAttribution(
  webDataRoot,
  {
    env = process.env,
    platform = process.platform,
  ...
  if (!ALLOWED_PYTHON_EXECUTABLES.has(pythonExecutable.toLowerCase())) {
    throw new ProductionDeploymentError("관측자료 출처 확인용 Python 실행기 이름이 올바르지 않습니다.");
  }
  const payload = await runPythonJson({
    argumentsList: [ATTRIBUTION_READER_PATH, "--root", path.resolve(webDataRoot)],
    env,
    errorMessage: "GCS WebUI 관측자료 출처를 읽을 수 없습니다.",
    pythonExecutable,
    spawnProcess,
    timeoutMessage: "GCS WebUI 관측자료 출처 확인 시간이 초과되었습니다.",
    timeoutMs
  });
  try {
    return Object.freeze(validatePublicObservationAttribution(payload));
  } catch {
    throw new ProductionDeploymentError("GCS WebUI 관측자료 출처가 공개 응답 계약과 맞지 않습니다.");
  }
}
...
function runPythonJson({
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout?.on("data", (chunk) => appendBoundedOutput(stdout, chunk));
    child.stderr?.on("data", (chunk) => appendBoundedOutput(stderr, chunk));
    child.once("error", () => finish(() => reject(new ProductionDeploymentError(errorMessage))));
    child.once("close", (code) => finish(() => {
      if (code !== 0) {
        const detail = Buffer.concat(stderr).toString("utf8").trim();
        reject(new ProductionDeploymentError(detail || errorMessage));
        return;
      }
      try {
        resolve(JSON.parse(Buffer.concat(stdout).toString("utf8").replace(/^\uFEFF/u, "")));
      } catch {
        reject(new ProductionDeploymentError(errorMessage));
      }
    }));
    timer = setTimeout(() => {
      child.kill?.("SIGKILL");
      finish(() => reject(new ProductionDeploymentError(timeoutMessage)));
    }, timeoutMs);
    timer.unref?.();
  });
}
...
async function resolveExistingDirectories(fileSystem, mountRoot, webDataRoot) {
  try {
    ...
    throw new ProductionDeploymentError("GCS 마운트 또는 자료판 디렉터리를 확인할 수 없습니다.");
  }
}
