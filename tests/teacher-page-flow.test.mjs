...
test("기존 학생 카드 정렬과 인증 진입점 및 동적 자료판 전달을 보존한다", () => {
  assert.match(styleSource, /\.preset-grid button:not\(\.custom-preset\)/u);
  assert.match(indexSource, /src="\.\/access-gate\.js"/u);
  assert.doesNotMatch(indexSource, /(?:src|href)="\.\/public-app\.(?:css|js)"/u);
  assert.doesNotMatch(indexSource, /access-gate\.js[?#]/u);
