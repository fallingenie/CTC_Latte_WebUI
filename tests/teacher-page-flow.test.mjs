...
test("기존 학생 카드 정렬과 Vite 해시 진입점 및 동적 자료판 전달을 보존한다", () => {
  assert.match(styleSource, /\.preset-grid button:not\(\.custom-preset\)/u);
  assert.match(indexSource, /href="\.\/public-app\.css"/u);
  assert.match(indexSource, /src="\.\/public-app\.js"/u);
  assert.doesNotMatch(indexSource, /public-app\.(?:css|js)[?#]/u);
